...
from utils import load_vocab, read_corpus, load_model, save_model, F1_score
...
def train(**kwargs):
    ...
    train_data = read_corpus('train', max_length=config.max_length, label_dic=label_dic, vocab=vocab)
    dev_data = read_corpus('validation', max_length=config.max_length, label_dic=label_dic, vocab=vocab)
    ...
    for epoch in range(config.base_epoch):
        ...
        loss_temp1 = dev(model, train_loader, epoch, config)
...
def dev(model, dev_loader, epoch, config):
    ...
    for i, batch in enumerate(dev_loader):
        ...
        loss = model.loss(feats, masks, tags)
        eval_loss += loss.item()
        best_path = best_path.reshape(-1)
        tags = tags.reshape(-1)
        pred.extend(best_path.cpu().numpy().tolist())
        true.extend(tags.cpu().numpy().tolist())
    print('eval  epoch: {}|  loss: {}'.format(epoch, eval_loss/length))
    print('f1_score(weighted): ', F1_score(pred, true))
    model.train()
    return eval_loss
