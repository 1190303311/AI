...
def read_corpus(name, max_length, label_dic, vocab):
    """
    :param path:数据文件路径
    ...
    sentences = data_set['tokens'][:1600]
    tags = data_set['ner_tags'][:1600]
    result = []
    for tokens, label in zip(sentences, tags):
        if len(tokens) > max_length-2:
            tokens = tokens[0:(max_length-2)]
            label = label[0:(max_length-2)]
        tokens_f =['[CLS]'] + tokens + ['[SEP]']
        label_f = [label_dic["<start>"]] + label + [label_dic['<eos>']]
...
def F1_score(pred, tag):
    return f1_score(tag, pred, average='weighted')